...
## 4. コマンド実行環境規定（Windows PowerShell 5.1）
- 実行環境は Windows PowerShell 5.1 である。
- `&&`, `||`, `;` 等によるコマンド連結を全面禁止とする（`&&` は構文エラーとなり、`;` は失敗時にも後続が強行されるため）。
- コマンドは必ず「1行につき1コマンドのみ単独実行」し、終了コード（code 0）を確認した上で次のコマンドへ進むこと。
