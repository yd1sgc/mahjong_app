...
1. **既存安全フック・スクリプトの継続利用:**
   - 危険なGit操作やDB直接破壊を監視する `scripts/safety_hook.py` をそのまま継続。
   - クラウドデータ退避用の `scripts/backup_supabase.py` を移行前後のバックアップに活用。
...
## 13. 成績分析・実力推定数理設計（MMC準拠統計モデル）

### 13.1 数理モデル概要
麻雀は1半荘あたりの分散（運のブレ）が非常に大きいゲームであるため、単なる累積平均ポイントではなく、中心極限定理に基づく正規分布モデルを適用して「真の実力（無限回対局時の期待収支 $\mu$）」を区間推定する。

### 13.2 統計量の計算式
1. **標本平均**: $\bar{X} = \frac{1}{N} \sum_{i=1}^N X_i$
2. **不偏標本標準偏差**: $s = \sqrt{\frac{1}{N-1} \sum_{i=1}^N (X_i - \bar{X})^2}$ （$N \le 1$ のときは $s = 0$）
3. **平均値の標準誤差**: $SE = \frac{s}{\sqrt{N}}$
4. **期待収支達成確率**:
   $$Z = \frac{X - \bar{X}}{SE}, \quad P(\mu \ge X) = 1 - \Phi(Z) = \Phi\left(\frac{\bar{X} - X}{SE}\right)$$
5. **確率密度関数（PDF）**:
   $$f(x) = \frac{1}{\sqrt{2\pi} SE} \exp\left( - \frac{(x - \bar{X})^2}{2 SE^2} \right)$$
   （横軸レンジ: $-10\text{pt} \sim +10\text{pt}$、刻み $0.5$、計41点）

### 13.3 誤差関数・正規累積分布の実装
外部ライブラリを排除し、純粋TypeScript関数として高精度多項式近似（Abramowitz & Stegun 7.1.26 準拠、最大誤差 $1.5 \times 10^{-7}$）を採用。
$$\Phi(z) = \frac{1}{2} \left[ 1 + \text{erf}\left(\frac{z}{\sqrt{2}}\right) \right]$$

### 13.4 同卓時直接対決・優位確率の計算式
同一半荘に参加した対局群のみを抽出し、半荘ごとのポイント差分 $d_i = X_{\text{自分}, i} - X_{\text{相手}, i}$ を標本として対応のある差分検定を実施。
$$SE_d = \frac{s_d}{\sqrt{N_{\text{同卓}}}}, \quad P(\text{相手より強い}) = \Phi\left(\frac{\bar{d}}{SE_d}\right)$$
（$SE_d = 0$ の境界値では $\bar{d} > 0 \implies 100\%$, $\bar{d} < 0 \implies 0\%$, $\bar{d} = 0 \implies 50\%$）

### 13.5 単一ルール制限（母集団整合性の担保）
ウマ・オカが異なるルールを混在集計すると分散のスケールが崩壊するため、ルールフィルターで単一ルールが選択されている場合のみ計算・描画を実行し、全ルール選択時は注意メッセージを表示して計算を安全に停止する。

---

## 14. 局詳細成績・局収支・親子対比設計

### 14.1 局収支（1局平均得失点）の数理設計
各局における持ち点増減（本場、供託棒、ノーテン罰符を含む実収支）を対象に、1局あたりの期待得失点を四捨五入整数で算出する。
- **全局局収支**: $\displaystyle \text{round}\left(\frac{\sum \text{score\_delta}}{\text{全局数}}\right)$
- **親番局収支**: $\displaystyle \text{round}\left(\frac{\sum_{\text{親番}} \text{score\_delta}}{\text{親番局数}}\right)$
- **子番局収支**: $\displaystyle \text{round}\left(\frac{\sum_{\text{子番}} \text{score\_delta}}{\text{子番局数}}\right)$
- **色彩規律**: プラス値はシアン（`text-cyan-400 font-black`）、マイナス値はローズ（`text-rose-400 font-black`）、0はニュートラルグレー（`text-neutral-500`）。

### 14.2 親番・子番の自動判別仕様
- **局名解析**: `rounds.kyoku_name` から正規表現 `/[東南西北](\d)局/` により局番号 $k \in \{1, 2, 3, 4\}$ を抽出。
- **座席照合**: `round_seats.seat`（1: 起家東家, 2: 起家南家, 3: 起家西家, 4: 起家北家）と照合し、`seat === k` を親番、`seat !== k` を子番と判定。
- **連荘の不変性**: 連荘中も局名は変化しない（本場数のみ増加）ため、連荘局を含めて親番・子番が厳密に分離集計される。

### 14.3 詳細成績テーブル 6タブ構成（`StatsDetailsTab.tsx`）
1. **基本**: 局数、和了率、ツモ率、放銃率、和銃差、テンパイ率、ノーテン罰符、供託収支
2. **打点**: 平均和了、立直平均打点、副露平均打点、ダマ平均打点、打点効率、**局収支**
3. **守備**: 放銃率、被リーチ放銃率、被副露放銃率、被ダマ放銃率、平均放銃
4. **立直**: リーチ率、立直和了率、立直放銃率
5. **副露**: 副露率、副露和了率、副露放銃率、ダマ和了率
6. **親子（新設・代替案A）**: 1段・全12列横スクロール（行高さ完全一致・画面ガタつきゼロ）
   - カラム: `名前` | `親局` | `子局` | `連荘率` | `親局収支` | `子局収支` | `親和了` | `子和了` | `親放銃` | `子放銃` | `親平均点` | `子平均点`
   - 連荘率定義: `(親和了回数 + 流局親聴牌回数) / 親番局数`（%）
   - 平均打点定義: 和了手役素点 `base_point` の平均（本場・供託棒を含まない純粋打点）
