...
- **Phase 5-H完了（成績画面フィルター正常化 & ゲスト表示切り替え配備 & 供託収支復元 & ＋符号排除）:**
  ...
  - **供託収支カラムの復元 & ＋符号排除（マイナスのみ表示）**:
    - `RoundSeatItem` に `kyotaku_point`、`RoundStatsRow` に `kyotakuPoint` を追加し、詳細成績「基本」タブに「供託収支」列を復元。
    - 試合成績表、詳細成績表、対局履歴、モーダル、推移グラフ、相性マトリクスの全画面で、正の数値から `+` 記号を完全排除し、マイナスのみ `-` 表示に統一。
  - **フィルター変更時の連動不整合解消**:
