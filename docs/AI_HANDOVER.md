...
- **Phase 5-H完了（成績画面フィルター正常化 & ゲスト表示切り替え配備 & 供託収支復元 & ＋符号排除）:**
  ...
  - **供託収支・打点・放銃点集計の正本SQL仕様完全準拠（`src/lib/mahjong/statsCalc.ts`）**:
    ...
    - 平均打点・平均放銃点: `score_delta`（本場・供託を含む局収支）の誤参照を撤廃し、純粋な手役素点である **`base_point`（基本素点）** による和了点・放銃点集計（`SUM(CASE WHEN rs.is_winner=1 THEN rs.base_point...)`）へ完全一致化。
    - ノーテン罰符収支: DBの `round_seats.penalty_point` を直接合計する正本仕様に統一。
    - 守備スタッツ: 局コンテキスト（局内に立直者・副露者がいたか）に基づき、被リーチ・被副露・被ダマ放銃を正しく判定。
...
- **Phase 5-I完了（ノーテン罰符集計リファクタリング & 成績表示文字色ミニマル化）:**
  - **ノーテン罰符計算の構造的リファクタリング（`src/lib/mahjong/statsCalc.ts`）**:
    - 流局時の席フィルタリングおよび二重ループ処理を完全撤廃。
    - 局の先頭でテンパイ人数（`tenpaiCount`）を確定し、単一の席ループ（`for (const s of seatsWithNames)`）内の流局処理へノーテン罰符（場3,000点配分: 1人聴牌+3000/-1000、2人聴牌+1500/-1500、3人聴牌+1000/-3000）を完全統合。
    - DBの `penalty_point` 直接参照による異常値（+25等）およびブロック境界不整合による構文エラー（TS1005）を構造的に解消。
  - **成績テーブル文字色のミニマル化（`GameStatsTable.tsx`, `StatsDetailsTab.tsx`）**:
    - オカなしPt、順位率（1着率〜4着率）、和銃差、ノーテン罰符、供託収支、打点効率、平均放銃、立直和了率、立直放銃率、副露和了率、副露放銃率、およびテーブルヘッダーの過剰なカラー装飾をすべて削除し、標準文字色（`text-neutral-300` / `text-neutral-400`）へ統一。
  - `npx vitest run`: 全44件 ALL PASS。
  - `npx tsc --noEmit`: 型エラー 0件。
