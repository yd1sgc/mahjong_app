...
export default function StatsPage() {
  ...
  return (
    <main className="w-full min-h-screen bg-black text-white max-w-xl mx-auto p-4 flex flex-col gap-5 pb-16">
      ...
      {error && (
        <div className="p-4 rounded-xl bg-rose-950/40 border border-rose-800 text-rose-300 text-xs font-bold">
          {error}
        </div>
      )}

      {loading ? (
        <div className="p-16 text-center text-neutral-500 text-xs font-bold">
          成績データを集計中...
        </div>
      ) : (
        <>
