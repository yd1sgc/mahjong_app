...
- **Phase 5-H完了（成績画面フィルター正常化 & ゲスト表示切り替え配備）:**
  - **ルールフィルターの正常化（`src/app/stats/page.tsx`）**:
    - DBのV2スキーマ仕様に準拠し、架空の `rule_id` 参照を廃止して `rule_name_snapshot`（実対局名称）による直接照合に修正。
    - 対局データおよびルールテンプレートに存在するユニーク名称からセレクト選択肢を動的生成。
