...
- **個人成績・詳細スタッツ（6タブ構成）:**
  ...
  - 局詳細データに基づく高度な分析（基本、打点、守備、立直、副露、親子）。
  - 「打点」タブ: 平均和了、立直・副露・ダマ平均打点、打点効率、**局収支（1局平均得失点）**。
  - 「親子」タブ: 代替案A（1段・12列横スクロール）による親番・子番の成績対比（親局、子局、連荘率、親局収支、子局収支、親和了率、子和了率、親放銃率、子放銃率、親平均点、子平均点）。
- **過去対局詳細の閲覧:**
  - 対局開始時点のルールスナップショットに基づき、過去の全対局・全局の推移を不変の状態で表示。
- **実力推定・同卓者分析（MMC準拠統計モデル）:**
  - 中心極限定理に基づく正規分布モデルにより、運の要素を排除した真の期待収支（1半荘あたり期待pt）と標準誤差 $SE = s / \sqrt{N}$ を推定。
  - 実力「0」以上の確率（長期プラス収支達成度）から「5」以上の確率までを客観パーセント表示。
  - 単一ルール選択時のみ動作（ウマ・オカ混在による統計的破綻を防止）。
  - 同卓者との直接対決比較: 同卓数順リスト、成績対比、差分検定による「相手より強い確率（優位確率）」、2つの正規分布山を重ね合わせた比較SVGグラフ描画。
  - デフォルト折りたたみのアコーディオン仕様により初期描画負荷をゼロ化。
